...
import logging
import os
log = logging.getLogger('zen.ZenNeighbors')

import ZenPacks.Rackspace.Neighbors
...
OperatingSystem._relations += (('neighborslinks', ToManyCont(ToOne, 'ZenPacks.Rackspace.Neighbors.NeighborsLink', 'os')),)


# Register python class
def initialize(registrar):
    registrar.registerClass(Policy.Policy, permission='Add DMD Objects',)
...
class ZenPack(ZenPackBase):
    ...
    def install(self, app):
        ...
        # Install menus
        self.installMenuItems(app.zport.dmd, menuName='Neighbors')
        ...
        self.addModeler(app.zport.dmd, organizer='/Network/Cisco', modelerNames=['Rackspace.Neighbors.CDPNeighborsMap'])
    ...
    def remove(self, app, leaveObjects=False):
        ...
        # Remove menus
        self.removeMenuItems(app.zport.dmd, menuName='Neighbors')

        if not leaveObjects:
            # Remove components based on classes extended by this package
            self.removeParts(app.zport.dmd, componentTypes='os', componentNames=['neighborslinks'])

            # Remove report organizers
            self.delReportOrg(app.zport.dmd, parent=None, organizer=None)

            # Remove modeler from organizer
            self.removeModeler(app.zport.dmd, organizer='/Network/Cisco', modelerNames=['Rackspace.Neighbors.CDPNeighborsMap'])
    ...
    def recatalog(self, dmd, dontReIndex=0):
        try:
           ...
           log.warn('CATALOG: Attribute dontReindexMyStuffZenpack created and set to 0. Toggle to 1 to skip future catalog operations.')
